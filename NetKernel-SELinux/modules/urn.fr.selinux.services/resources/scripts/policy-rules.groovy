/**
 * Ce script recherche les règles d'accès ou d'audit concernant l'étiquette indiquée,
 * dans la politique dont l'identifiant est passé en paramètre.
 *
 * Si arg:rules vaut "access", il cherche l'étiquette dans les règles "allow" et "neverallow".
 * Si arg:rules vaut "audit", il cherche l'étiquette dans les règles "dontaudit" et "auditallow".
 * 
 * Il utilise l'index "onAccessRules" ou "onAuditRules" sur res:/services/policies/<policyname>/query/
 * puis renvoie la ou les instructions qui font référence à cette étiquette.
 */
...
// Parameter 'domain'
String paramDomain = context.source("arg:domain");

// Parameter 'rules'
String paramRules = context.source("arg:rules");
...
if (! myPolicy.getReader().getFirstValue("/output/status").equals("success")) {
    ...
    // We look for access or audit rules that use the domain or type
    List<IHDSReader> listReaders;
    if ("accessrules".equals(paramRules)) {
    	listReaders = myPolicy.getReader().getNodes("key('onAccessRules','"+paramDomain+"')");
    }
    else if ("auditrules".equals(paramRules)) {
    	listReaders = myPolicy.getReader().getNodes("key('onAuditRules','"+paramDomain+"')");
    }
